"""
Test Game 3 scenario with Nash Equilibrium analysis.
...
GAME_3_BANS = [
    ...
    "Yone", "Azir", "Orianna", "Cassiopeia", "Zoe", "Galio", "Trundle", "Pantheon", "Bard", "Lillia" # Game 3 actual bans
]

# KT (Red Team 1) - 3 picks locked, 2 open (BOT and SUPPORT)
GAME_3_KT_LOCKED = {
    "TOP": "Ksante",
    "JUNGLE": "DrMundo", 
    # Mid is open
...
# T1 (Blue Team 2) - 4 picks locked, 1 open (TOP)
GAME_3_T1_LOCKED = {
    "TOP": "Renekton",
     # "JUNGLE": is open
     # "MID": is open
...
def create_game3_draft_state():
    """Create draft state for Game 3.
    ...
    # Normalize bans
    normalized_bans = normalize_champion_list(GAME_3_BANS)
    
    # Normalize locked picks
    # KT is Red Team (team1)
    normalized_kt = {}
    for role, champ in GAME_3_KT_LOCKED.items():
    ...
    normalized_t1 = {}
    for role, champ in GAME_3_T1_LOCKED.items():
        normalized = normalize_champion_name(champ)
        if normalized:
    ...
    draft = create_draft_from_scenario(
        ...
        team1_last_role="MID",  # Red picks MID last
        team2_picks=normalized_t1,  # T1 (Blue)
        team2_last_role="MID"  # Blue picks JUNGLE + MID (we'll handle JUNGLE separately)
    )
...
def run_nash_equilibrium_analysis():
    """Run Nash equilibrium analysis for Game 3.
    T1 is Blue Team, KT is Red Team.
    Blue picks JUNGLE + MID, Red picks MID.
    """
    print("=" * 70)
    print("GAME 3 NASH EQUILIBRIUM ANALYSIS")
    print("=" * 70)
    ...
    # Restrict to scenario's extended MID pool
    mid_champions_red = [c for c in EXTENDED_CHAMPION_POOL["MID"] if c in available]
    
    # Count how many JUNGLE and MID champions are available for Blue's picks
    jungle_champions_blue = [c for c in available if scorer.get_champion_data(c) and scorer.get_champion_data(c).get('is_jungle', 0) == 1]
    ...
    print(f"  Beam width: {spne.beam_width} (set high to prevent pruning - we only have ~{len(mid_champions_blue)} Blue MID choices)")
    ...
    print(f"  Available MID champions for Red: {len(mid_champions_red)}")
    print(f"  Available JUNGLE champions for Blue: {len(jungle_champions_blue)}")
    print(f"  Available MID champions for Blue: {len(mid_champions_blue)}")
    print(f"  Total combinations Blue can pick (MID only): {len(mid_champions_blue)}")
    print(f"  Expected calculations: ~{len(mid_champions_blue) * len(mid_champions_red)} (all combinations)")
    print(f"  NOTE: With beam_width={spne.beam_width}, all {len(mid_champions_blue)} Blue MID choices should be explored")
    
    print(f"  Mid champions (Red): {', '.join(mid_champions_red[:10])}{'...' if len(mid_champions_red) > 10 else ''}")
    ...
    try:
        ...
        # Get all valid JUNGLE and MID champions
        ...
        for champ in available:
            ...
            if champ_data:
                if champ_data.get('is_jungle', 0) == 1:
                    valid_jungle_champions.append(champ)
        # Restrict valid mids to scenario's extended MID pool
        for champ in EXTENDED_CHAMPION_POOL["MID"]:
            if champ in available:
                valid_mid_champions.append(champ)
        
        print(f"Found {len(valid_jungle_champions)} valid JUNGLE champions")
        print(f"Found {len(valid_mid_champions)} valid MID champions (scenario pool)")
        print(f"Total combinations to test (MID only): {len(valid_mid_champions)}\n")
        ...
        # OPTIMIZATION: Pre-calculate DrMundo vs all Blue JUNGLE champions
        # We'll invert the winrate when we need JUNGLE vs DrMundo
        drmundo_champ = red_team[1]  # DrMundo is Red's locked JUNGLE
        print(f"\nPre-calculating {drmundo_champ} vs all {len(valid_jungle_champions)} Blue JUNGLE champions...")
        print(f"  This will make {len(valid_jungle_champions)} API calls, then we'll invert for JUNGLE vs DrMundo")
        
        drmundo_vs_jungle_matchups = {}
        successful_drmundo_matchups = 0
        with tqdm(total=len(valid_jungle_champions), desc="Pre-calc DrMundo vs JUNGLE", unit="champ") as pbar:
            for jungle_champ in sorted(valid_jungle_champions):
                # Get DrMundo vs JUNGLE winrate (this populates the cache)
                wr = spne.scorer.get_champion_matchup_winrate(drmundo_champ, jungle_champ, 'jungle')
                drmundo_vs_jungle_matchups[jungle_champ] = wr
                if wr is not None:
                    successful_drmundo_matchups += 1
                pbar.update(1)
                if wr is not None:
                    pbar.set_postfix({'last': f"{jungle_champ}: {wr:.2%}", 'cache': len(spne.scorer.matchup_cache)})
        
        print(f"Pre-calculated {successful_drmundo_matchups}/{len(valid_jungle_champions)} DrMundo vs JUNGLE matchups (cache now has {len(spne.scorer.matchup_cache)} entries)")
        ...
        # Check only champions in scenario's extended MID pool
        for champ in EXTENDED_CHAMPION_POOL["MID"]:
        ...
        print(f"Found {len(valid_mid_champions_red)} valid MID champions for Red (scenario pool)")
        ...
        # Test JUNGLE vs DrMundo (should be cached via inversion)
        test_jungle = valid_jungle_champions[0] if valid_jungle_champions else None
        if test_jungle:
            cache_before = len(spne.scorer.matchup_cache)
            wr_test = spne.scorer.get_champion_matchup_winrate(test_jungle, drmundo_champ, 'jungle')
            cache_after = len(spne.scorer.matchup_cache)
            if cache_after == cache_before:
                test_cache_hits += 1
                print(f"  ✓ {test_jungle} vs DrMundo (JUNGLE): CACHE HIT (inverted)")
            else:
                test_cache_misses += 1
                print(f"  ✗ {test_jungle} vs DrMundo (JUNGLE): CACHE MISS (unexpected!)")
        ...
        print(f"\nTotal calculations needed: {len(valid_mid_champions) * len(valid_mid_champions_red)}")
        print(f"  ({len(valid_mid_champions)} MID × {len(valid_mid_champions_red)} MID)")
        print(f"  Expected cache hits: All locked roles (TOP, BOT, SUPPORT), all JUNGLE vs DrMundo (inverted)")
        print(f"  Expected new API calls: MID vs MID matchups (both sides are variable, cannot pre-calc)\n")
        
        blue_combinations = []
        
        print("=" * 70)
        print("ALL BLUE COMBINATIONS (MID only):")
        print("=" * 70)
        ...
        print(f"Calculating {total_combos} Blue MID choices × {len(valid_mid_champions_red)} Red MID picks...")
        ...
        mid_combos = []
        for mid_champ in sorted(valid_mid_champions):
            mid_combos.append(mid_champ)
        
        # Calculate all Blue combinations (no progress bar - should be fast with caching)
        # Fix Blue JUNGLE to a single pick (use actual if provided, else first available)
        fixed_jungle = blue_team[1] or normalize_champion_name(GAME_3_T1_ACTUAL.get("JUNGLE", "")) or (valid_jungle_champions[0] if valid_jungle_champions else None)
        for mid_champ in mid_combos:
            ...
            test_blue[1] = fixed_jungle  # JUNGLE fixed
            ...
            blue_combinations.append((jungle_champ, mid_champ, test_payoff, t1_win, best_red_mid_for_combo))
        ...
        optimal_t1_jungle = minimax_jungle
        optimal_t1_mid = minimax_mid
        optimal_kt_mid = minimax_red_mid
        ...
        for i, combo in enumerate(blue_combinations[:10]):
            jungle, mid, payoff, win, red_mid = combo
            marker = " [SPNE]" if jungle == optimal_t1_jungle and mid == optimal_t1_mid else ""
            advantage = "Blue advantage" if payoff > 0 else "Red advantage"
            print(f"  {i+1:2d}. JUNGLE={jungle:<15} MID={mid:<15} Payoff={payoff:>+10.4f} ({advantage}), T1 Win={win:>6.2%}, Red MID={red_mid}{marker}")
        ...
        print(f"  Formula: {len(valid_jungle_champions)} JUNGLE × {len(valid_mid_champions)} MID × {len(valid_mid_champions_red)} MID = {len(valid_jungle_champions) * len(valid_mid_champions) * len(valid_mid_champions_red)} total")
        ...
        actual_blue_team[1] = t1_actual_jungle  # JUNGLE
        actual_blue_team[2] = t1_actual_mid  # MID
        
        print(f"\n{TEAM_2['name']} (Blue/T1) - Actual Picks:")
        print(f"  JUNGLE:  {t1_actual_jungle}")
        ...
        actual_red_team[2] = kt_actual_mid  # MID
        ...
        scenarios = [
            (f"Optimal ({optimal_t1_jungle}, {optimal_t1_mid}) vs Optimal ({optimal_kt_mid})", optimal_blue_team, optimal_red_team),
            (f"Actual ({t1_actual_jungle}, {t1_actual_mid}) vs Actual ({kt_actual_mid})", actual_blue_team, actual_red_team),
            (f"Optimal ({optimal_t1_jungle}, {optimal_t1_mid}) vs Actual ({kt_actual_mid})", optimal_blue_team, actual_red_team),
            (f"Actual ({t1_actual_jungle}, {t1_actual_mid}) vs Optimal ({optimal_kt_mid})", actual_blue_team, optimal_red_team),
        ]
        ...
        print(f"  JUNGLE:  Actual={t1_actual_jungle}, Optimal={optimal_t1_jungle}, Match={t1_actual_jungle == optimal_t1_jungle}")
        ...
        print(f"\n1. Optimal ({optimal_t1_jungle}, {optimal_t1_mid}) vs Optimal ({optimal_kt_mid}) (SPNE):")
        print(f"   Payoff: {opt_vs_opt[1]:+.4f}, T1: {opt_vs_opt[2]:.1%}, KT: {opt_vs_opt[3]:.1%}")
        
        print(f"\n2. Actual ({t1_actual_jungle}, {t1_actual_mid}) vs Actual ({kt_actual_mid}) (What happened):")
        ...
        print(f"\n3. Optimal ({optimal_t1_jungle}, {optimal_t1_mid}) vs Actual ({kt_actual_mid}) (If Blue optimal, Red actual):")
        ...
        print(f"\n4. Actual ({t1_actual_jungle}, {t1_actual_mid}) vs Optimal ({optimal_kt_mid}) (If Blue actual, Red optimal response):")
        ...
        kt_best_response_to_optimal = None
        ...
        if kt_best_response_to_optimal != optimal_kt_mid:
            minimax_response = next((p for c, p, w in kt_responses_to_optimal if c == optimal_kt_mid), None)
            if minimax_response is not None:
                minimax_payoff_red = minimax_response
                print(f"  Minimax pick was: {optimal_kt_mid} (payoff: {minimax_payoff_red:+.4f})")
        ...
        kt_best_response_to_actual = None
        ...
        for champ in sorted(valid_mid_champions_actual):
            ...
            if test_payoff < best_kt_payoff_to_actual:
                ...
                kt_best_response_to_actual = champ
        ...
        for i, (champ, payoff, win) in enumerate(kt_responses_to_actual[:5]):
            marker = " [BEST]" if i == 0 else " [ACTUAL]" if champ == kt_actual_mid else ""
        ...
        optimal_vs_br_optimal_red[2] = kt_best_response_to_optimal  # MID
        ...
        actual_vs_br_actual_red[2] = kt_best_response_to_actual  # MID
        ...
        scenarios = [
            (f"Optimal Blue ({optimal_t1_jungle}, {optimal_t1_mid}) vs SPNE Red ({optimal_kt_mid})", optimal_blue_team, optimal_vs_spne_red),
            (f"Optimal Blue ({optimal_t1_jungle}, {optimal_t1_mid}) vs Best Response ({kt_best_response_to_optimal})", optimal_blue_team, optimal_vs_br_optimal_red),
            (f"Actual Blue ({t1_actual_jungle}, {t1_actual_mid}) vs Best Response ({kt_best_response_to_actual})", actual_blue_team, actual_vs_br_actual_red),
            (f"Actual Blue ({t1_actual_jungle}, {t1_actual_mid}) vs Actual Red ({kt_actual_mid})", actual_blue_team, actual_vs_actual_red),
        ]
        ...
        print(f"\n1. Blue's Optimal Picks ({optimal_t1_jungle}, {optimal_t1_mid}): JUNGLE={optimal_t1_jungle}, MID={optimal_t1_mid}")
        print(f"   Red's SPNE response ({optimal_kt_mid}): {optimal_kt_mid}")
        ...
        if kt_best_response_to_optimal == optimal_kt_mid:
            print(f"   [OK] SPNE pick ({optimal_kt_mid}) matches best response ({kt_best_response_to_optimal})!")
        else:
            ...
            print(f"   [WARNING] SPNE pick ({optimal_kt_mid}) is NOT the best response ({kt_best_response_to_optimal})!")
        ...
        print(f"\n2. Blue's Actual Picks ({t1_actual_jungle}, {t1_actual_mid}): JUNGLE={t1_actual_jungle}, MID={t1_actual_mid}")
        ...
        print(f"   Red's actual pick ({kt_actual_mid}): {kt_actual_mid}")
        print(f"     -> Payoff: {actual_vs_actual[1]:+.4f} (T1 win: {actual_vs_actual[2]:.1%}, KT win: {actual_vs_actual[3]:.1%})")
        
        if kt_best_response_to_actual == kt_actual_mid:
        ...
        print(f"  If Blue picks optimally ({optimal_t1_jungle}, {optimal_t1_mid}), then")
        print(f"  Red's SPNE pick ({optimal_kt_mid}) MUST be the best response (minimum payoff).")
        
        if kt_best_response_to_optimal == optimal_kt_mid:
            print(f"\n[OK] SPNE VERIFIED: The SPNE pick ({optimal_kt_mid}) IS the best response ({kt_best_response_to_optimal})!")
            ...
            print(f"  SPNE calculated ({optimal_kt_mid}): {optimal_kt_mid}")
            print(f"    -> Payoff: {opt_vs_spne[1]:+.4f} (T1 win: {opt_vs_spne[2]:.1%}, KT win: {opt_vs_spne[3]:.1%})")
